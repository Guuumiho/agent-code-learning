...
const STATIC_MODE = Boolean(window.STATIC_ANALYSIS_DATA);
...
async function getFileContent(filePath) {
  ...
  if (STATIC_MODE) {
    throw new Error("静态站未包含这个文件的源码内容。请重新导出最新历史分析。");
  }
...
async function bootApp() {
  if (STATIC_MODE) {
    ...
    setSettingsOpen(false);
    await initEditor();
    await applyAnalysisData(window.STATIC_ANALYSIS_DATA, "静态站已加载");
    return;
  }
  loadHistoryList();
  initEditor().catch(() => {});
